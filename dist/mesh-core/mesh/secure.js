...
import { packJsonForTransport, unpackJsonFromTransport, } from '../json-transport.js';
export function createSecureChannel(passphrase, iterations) {
    const options = { passphrase, iterations };
    return {
        seal: (value) => packJsonForTransport(value, options),
        open: (envelope) => unpackJsonFromTransport(envelope, options),
    };
}
