...
import { createSecureChannel } from './secure.js';
import { packBody, packBodySmallest, unpackBody, compressionSupported, encodedSize } from './compress.js';
...
export class MeshNode {
    constructor(opts = {}) {
        ...
        Object.defineProperty(this, "peerToTransport", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        ...
        Object.defineProperty(this, "serviceObjects", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        ...
        Object.defineProperty(this, "discoveryTtl", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "heartbeatMs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "compressThreshold", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "bodyCodec", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "gossipFanout", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    async stop() {
        ...
        for (const s of this.serviceObjects) {
            try {
                ...
                console.error(`[mesh] service "${s.name}" detach failed`, err);
            }
        }
        for (const t of this.transports) {
            try {
    ...
    request(to, type, body, timeoutMs = 8000) {
    ...
    async ping(to, timeoutMs = 5000) {
        const t0 = Date.now();
        const reply = await this.request(to, 'mesh.ping', {}, timeoutMs);
    ...
    originate(opts) {
        ...
        if (this.secure && !isControl) {
            ...
            const pick = bodyCodec === 'auto'
                ...
                : packBody(env.body, bodyCodec).then((data) => ({ codec: bodyCodec, data }));
            pick
                .then(({ codec, data }) => this.route({ ...env, zip: true, ...(codec === 'gzip' ? {} : { zc: codec }), body: data }))
    ...
    route(env) {
        ...
        if (route)
            this.transmitTo(route.via, env);
        else
            this.flood(env, null);
    }
    onIncoming(frame, fromPeer) {
        const now = Date.now();
        const res = this.router.handle(frame, fromPeer, now);
        if (res.deliver)
            this.dispatch(res.deliver, fromPeer, now);
        if (res.forward) {
            const { env, plan } = res.forward;
            this.carry(env); // hold relayed frames too, so late-joiners still get them
            if (plan.kind === 'unicast')
                this.transmitTo(plan.via, env);
            else
                this.flood(env, plan.except);
        }
    }
    dispatch(env, fromPeer, now) {
        ...
        if (env.zip) {
            unpackBody(env.body, env.zc ?? 'gzip')
    ...
    deliver(env, _fromPeer, _now) {
        ...
        if (env.channel)
            this.channels.get(env.channel)?.forEach((h) => h(ctx));
    }
    sayHello() {
    ...
    recomputeRoutes() {
        ...
        for (const rec of this.peers.values()) {
            if (rec.direct)
                link(this.id, rec.id); // self ↔ our direct neighbours
            for (const n of rec.neighbors)
                link(rec.id, n); // peer ↔ its gossiped neighbours
        }
        const now = Date.now();
        ...
        for (const [dest, path] of shortestPaths(this.id, adjacency)) {
            ...
            if (via === undefined)
                continue; // can't resolve a handle → leave to reverse-path / flood
    ...
    carry(env) {
        if (!this.store.enabled)
            return;
        if (env.type === HELLO || env.type === BYE || env.type === 'mesh.ping' || env.reply)
            return;
    ...
    replayTo(peer) {
        ...
        for (const f of this.store.pending(Date.now())) {
            if (f.to === null || f.to === peer)
                t.sendTo(peer, f.frame);
        }
    }
    flood(env, except) {
        ...
        if (eligible.length <= this.gossipFanout) {
            for (const peer of eligible)
                this.peerToTransport.get(peer).sendTo(peer, frame);
            return;
        }
    ...
    transmitTo(peer, env) {
        const t = this.peerToTransport.get(peer);
        if (t)
            t.sendTo(peer, encode(env));
        else
            this.flood(env, null); // route stale — fall back to flood
    }
}
