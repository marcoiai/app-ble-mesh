 ...
 */
export function createInvite(node, opts = {}) {
    let key = node.groupKey;
    if (!key) {
        key = randomKey();
        node.setSecret(key);
    }
    const invite = { v: 1, key, group: opts.group, from: node.info.label };
...
export function peekInvite(blob) {
    const invite = JSON.parse(decodeURIComponent(escape(atob(blob.trim()))));
...
export function acceptInvite(node, blob) {
