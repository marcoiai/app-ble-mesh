...
export function shortestPaths(
...
): Map<NodeId, GraphRoute> {
  // Collect every node mentioned anywhere in the graph.
  const nodes = new Set<NodeId>([self]);
  ...
  for (const n of nodes) dist.set(n, n === self ? 0 : Infinity);

  const unsettled = new Set<NodeId>(nodes);
  while (unsettled.size > 0) {
    // Pop the unsettled node with the smallest tentative distance.
    let u: NodeId | undefined;
    let best = Infinity;
    ...
    if (u === undefined || best === Infinity) break; // rest is unreachable
    unsettled.delete(u);
  ...
  const routes = new Map<NodeId, GraphRoute>();
  for (const dest of nodes) {
    if (dest === self) continue;
    const d = dist.get(dest)!;
    if (!Number.isFinite(d)) continue; // no path
