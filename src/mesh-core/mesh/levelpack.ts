...
const keyToToken = new Map<string, number>(KEY_TOKENS.map((key, index) => [key, index]));
const stringToToken = new Map<string, number>(STRING_TOKENS.map((value, index) => [value, index]));
...
export function levelPack(value: unknown): Uint8Array {
  const writer = new BinaryWriter();
  writeValue(writer, normalize(value));
  return writer.finish();
}

export function levelUnpack(bytes: Uint8Array): unknown {
  const reader = new BinaryReader(bytes);
  const value = readValue(reader);
  if (!reader.done()) throw new Error('levelpack: trailing bytes');
  return value;
}

export function levelPackStats(value: unknown): LevelPackStats {
...
function writeValue(writer: BinaryWriter, value: Jsonish): void {
  if (value === null) {
    writer.u8(TAG_NULL);
    return;
  }
  ...
  if (writeSpecial(writer, value)) return;
...
function writeSpecial(writer: BinaryWriter, value: { [key: string]: Jsonish | undefined }): boolean {
  if (isChat(value)) {
    writer.u8(TAG_CHAT);
...
function readValue(reader: BinaryReader): Jsonish {
  const tag = reader.u8();
  if (tag === TAG_NULL) return null;
  if (tag === TAG_FALSE) return false;
  if (tag === TAG_TRUE) return true;
  if (tag === TAG_INT) return unzigzag(reader.varint());
  if (tag === TAG_FLOAT) return reader.f64();
  if (tag === TAG_STRING) return reader.string();
  if (tag === TAG_STRING_TOKEN) return STRING_TOKENS[reader.varint()] ?? '';
  if (tag === TAG_ARRAY) {
  ...
  if (tag === TAG_OBJECT) {
    ...
    for (let i = 0; i < length; i += 1) out[readKey(reader)] = readValue(reader);
    return out;
  }
  if (tag === TAG_CHAT) {
    return {
  ...
  if (tag === TAG_ENVELOPE) {
    ...
    const out: { [key: string]: Jsonish | undefined } = { v, id, type, from, to, ttl, path, ts, body };
    if (channel !== null) out.channel = channel;
    if (corr !== null) out.corr = corr;
    if (reply) out.reply = true;
    if (enc) out.enc = true;
    if (zip) out.zip = true;
    return out;
  }
...
function writeValueString(writer: BinaryWriter, value: string): void {
...
function writeRawString(writer: BinaryWriter, value: string): void {
  const bytes = textEncoder.encode(value);
  writer.varint(bytes.length);
  writer.bytes(bytes);
}

function writeNullableString(writer: BinaryWriter, value: Jsonish | undefined): void {
  if (typeof value !== 'string') {
    writer.u8(0);
    return;
  }
...
function readNullableString(reader: BinaryReader): string | null {
  return reader.u8() === 1 ? reader.string() : null;
}

function writeKey(writer: BinaryWriter, key: string): void {
...
function readKey(reader: BinaryReader): string {
  if (reader.u8() === 1) return KEY_TOKENS[reader.varint()] ?? '';
  return reader.string();
}

function isChat(value: { [key: string]: Jsonish | undefined }): value is {
  room: string;
  from: string;
  label: string;
  text: string;
...
function isGameEvent(value: { [key: string]: Jsonish | undefined }): value is {
  from: string;
  payload: Jsonish;
} {
...
function isEnvelope(value: { [key: string]: Jsonish | undefined }): value is {
  v: number;
  id: string;
  type: string;
  from: string;
...
class BinaryReader {
  ...
  u8(): number {
    if (this.pos >= this.bytes.length) throw new Error('levelpack: unexpected eof');
  ...
  varint(): number {
    ...
    for (;;) {
      ...
      shift += 7;
      if (shift > 56) throw new Error('levelpack: varint too large');
    }
  }

  f64(): number {
    if (this.pos + 8 > this.bytes.length) throw new Error('levelpack: unexpected eof');
  ...
  string(): string {
    ...
    if (this.pos + length > this.bytes.length) throw new Error('levelpack: unexpected eof');
